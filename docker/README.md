...
For convenience the image also bundles optional external solvers and
renderers that ProvSQL can invoke as **separate subprocesses** (never linked
into the extension): **d4**, **dsharp**, **weightmc**, **Graphviz** (`dot`),
and **graph-easy**. Each is distributed under **its own upstream license**
(e.g. Graphviz under EPL, graph-easy under the Perl license; the SAT/model
counters under their respective licenses, some research-only). The exact
upstream sources, versions and build steps are recorded in

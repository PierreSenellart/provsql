...
--    plain() says (otherwise a warning is raised, see limit_warning).
-- ---------------------------------------------------------------------------
...
CREATE TABLE sn_o4 AS SELECT a FROM sn_r UNION SELECT a FROM sn_s ORDER BY a OFFSET plain(1) LIMIT plain(2);
...
CREATE TABLE sn_o5 AS SELECT a FROM sn_r EXCEPT SELECT a FROM sn_w ORDER BY a DESC LIMIT plain(2);
...
-- (a truncation of the actual arm, with plain(): the filter of a rank is
-- tested in limit_rank)
CREATE TABLE sn_o6 AS (SELECT a FROM sn_r ORDER BY a DESC LIMIT plain(1))
  UNION SELECT a FROM sn_w ORDER BY a LIMIT plain(2);
...
CREATE TABLE sn_o7 AS SELECT a FROM sn_r UNION ALL SELECT a FROM sn_s ORDER BY a LIMIT plain(3);
...
CREATE TABLE sn_o8 AS SELECT a, -a AS b FROM sn_r UNION SELECT a, -a FROM sn_w ORDER BY b LIMIT plain(3);

\set ECHO none
\if `which d4v2 > /dev/null 2>&1 && echo true || echo false`
...
CREATE TABLE compile_to_ddnnf_dot_d4v2_q AS
SELECT compile_to_ddnnf_dot(provenance(), 'd4v2') AS dot
FROM personnel p1, personnel p2, personnel p3
WHERE p1.id=5 AND p2.id=6 AND p3.id=7;

SELECT remove_provenance('compile_to_ddnnf_dot_d4v2_q');

-- Structural assertions (the d-DNNF shape depends on d4v2's choices).
SELECT
...
FROM compile_to_ddnnf_dot_d4v2_q;

DROP TABLE compile_to_ddnnf_dot_d4v2_q;
\else
\echo 'SKIPPING: d4v2 not available'
\endif

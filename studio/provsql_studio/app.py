...
def create_app(
    *,
    ...
    def api_set_prob():
        """Give an input gate a probability it does not have yet.

        From provsql 1.13.0 a probability is written once, and this
        endpoint does not work around that: it writes one on a gate that
        has none, and lets the extension refuse a gate that already has
        one, passing the refusal's detail and hint back verbatim.

        Changing a probability means giving the row a fresh input gate
        (`provsql.replace_input`), which leaves the old gate in the
        circuit with its old value everywhere else it is used.  That is
        a deliberate rewrite of the data, not the click of an editor, so
        it stays in the user's hands rather than happening behind an
        inspector field."""
        import psycopg
        payload = request.get_json(silent=True) or {}
        try:
        ...
        try:
            ...
            return jsonify({
                ...
                # The write-once refusal carries its reasoning in the
                # detail and the recipe in the hint; dropping either
                # leaves the user at a dead end.
                "explanation": diag.message_detail if diag else None,

...
* [ ] **ProvSQL feature gaps** (from audit): triage of extension features that have no studio UI surface. Each row: feature; doc step that wants it; suggested surface.
  ...
  * **`to_provxml` export**: CS1 §12. Button on the circuit-mode toolbar.
  ...
  * **`add_provenance` / `remove_provenance` / `repair_key`** (CS5 §5): document as SQL recipes; the tooltip on the schema-panel `prov` pill could link to the recipe.

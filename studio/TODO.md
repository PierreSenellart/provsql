...
* [x] **Actionable 413 banner**: when a circuit exceeds the per-fetch cap, the sidebar shows a structured banner ("Circuit has X nodes; cap is Y (rendering at depth D)") instead of bare error text. Server response carries `depth_1_size` (root + direct children); the banner offers a single-click **"Render at depth 1, then expand interactively"** button only when `depth > 1` AND `depth_1_size ≤ cap` (so wide-bound circuits like Q3's `agg_token` lose the button entirely rather than promising a render that 413s again). Frontier-expand 413s route through the same banner.
* [x] **Stub scene on circuit-load 413**: the eval strip can still target the unrendered root after a 413, since evaluation only needs the token UUID, not a displayed DAG. The frontier-expand 413 path leaves the existing rendered scene alone (it's still the right eval target).
* [x] **Inspector probability + click-to-edit** for input/update gates: a `<dt>probability</dt><dd>…</dd>` row in the same `<dl>` as `uuid` / `depth` / `info1`, formatted with the Config panel's "Probability decimals". Clicking the value swaps it for a number input ; Enter fires `POST /api/set_prob` ; Esc / blur cancels ; out-of-range values and ProvSQL errors land inline. The implicit default `1.0` is shown verbatim (no special-casing).
* [x] **Inspector depth fix on frontier expand**: `mergeSubgraph` now rebases sub-DAG depths into the original-root frame (sub.root sits at the anchor's absolute depth, not at depth 0), so expanded nodes report the correct distance from the root in the inspector.
...
* [x] Which-provenance (lineage) added to the Compiled Semirings dropdown: `value="which"` dispatches to `provsql.sr_which` via `_SR_FUNCTIONS`, shown between Why and Formula; takes a mapping like the other text-valued compiled semirings; result renders in the same `text` chip path. Picked up after the `Which` semiring landed in the extension (master merge).
* [x] Tropical (min-plus) and Viterbi (max-times) added to the Compiled Semirings dropdown: `value="tropical"` / `value="viterbi"` dispatch to `provsql.sr_tropical` / `provsql.sr_viterbi` via `_SR_FUNCTIONS`, shown after Formula; both take a numeric mapping (cost or probability respectively) and surface a type hint inline (`Expects numeric (cost) values.` / `Expects numeric values in [0, 1].`); results render in the `float` chip path with the same rounded↔full toggle as `probability`. Picked up after the two compiled semirings landed in the extension (master merge).
* [x] Temporal (interval-union) added to the Compiled Semirings dropdown: `value="temporal"` dispatches to `provsql.sr_temporal` via `_SR_FUNCTIONS`, shown after Viterbi; takes a `tstzmultirange`-valued mapping (validity intervals) and surfaces an inline hint (`Expects tstzmultirange (validity-interval) values; PostgreSQL 14+.`). The result is a `tstzmultirange` rendered through the `text` chip path: psycopg's `Multirange.__str__` already produces the canonical `{[lo, hi), ...}` literal, which `_to_jsonable` passes through unchanged. PG14+ only since `sr_temporal` lives in `provsql.14.sql` (calling it on PG13 surfaces as a "function does not exist" error from the extension itself). Picked up after the temporal compiled semiring landed in the extension (master merge).
...
* [x] `tests/test_relations.py`: schema-panel `has_provenance` flag (positive on personnel, negative on a freshly-created untagged table); per-relation row-cap behaviour (truncated when over cap, full result when under); mapping-shape relations excluded from the where-mode sidebar even when they carry a planner-injected `provsql` column.
* [x] `tests/test_exec.py`: auto-prepare regression — same SELECT run 16+ times alternating wp on/off, asserting the UUIDs swing between two values (would hang at the wp=on UUID without `prepare_threshold = None`); result-row cap behaviour (truncated at cap=3, full at default 1000).
* [x] `tests/test_circuit.py`: 413 response carries `depth_1_size` (used by the front-end to decide whether the retry button is meaningful); `/api/leaf` includes the stored probability (default `1.0` when `set_prob` never ran, `0.42` after explicit set); `POST /api/set_prob` writes the value, rejects out-of-range / non-numeric / invalid-UUID payloads.
...
### Config panel reorganisation

* [x] Three sub-grouped sections (**Provenance** / **Session** / **Display limits**) replace the flat list ; the inline hint paragraphs are dropped (will be re-added later as `<?>`-style links to the online docs). The `Session settings` panel title is gone : the section headings carry the structure.
* [x] Three new size knobs in **Display limits**: "Sidebar rows per relation" (1..5000, default 100), "Result rows" (1..100 000, default 1000), "Nodes per fetch" (10..10 000, default 200). All persisted in `~/.config/provsql-studio/config.json` and threaded through CLI flags `--max-sidebar-rows` / `--max-result-rows` / `--max-circuit-nodes`. Changing "Nodes per fetch" clears the layout cache so prior-cached scenes don't survive the cap change.

### Developer tools

* [x] **Synthetic big-data fixture** under `studio/scripts/`: `load_demo_big.sql` (idempotent loader, fixed seed, ~50 000-row `bench_events` + 4-row `bench_dim` + a `bench_severity_mapping` provenance mapping, with per-row probabilities decreasing with severity); `big_demo_queries.sql` (eight canned queries hitting one stress axis each: result-row cap, filtered cap, agg fan-in 413, join circuit, shared-subgraph DAG, probability eval, mapping eval, where-mode sidebar). Used as the manual smoke test for every cap / banner / eval-strip change.
...
* [ ] **ProvSQL feature gaps** (from audit): triage of extension features that have no studio UI surface. Each row: feature; doc step that wants it; suggested surface.
  ...
  * **`shapley` / `banzhaf` / `*_all_vars`** (CS2 §13–15): deferred to a dedicated **Contributions mode** (third mode, after Where + Circuit; see Future work). The per-input contributions surface is a heat-map over input gates, structurally closer to where-provenance than to the eval strip. Not a v0.1 blocker.
  * **Temporal SRFs (`timeslice`, `history`, `timetravel`)** (CS4 §3–5): deferred to a dedicated **Time-travel mode** (fourth mode; see Future work). The SRF call shape (`... AS (cols ...)`) plus the date/window/filter inputs warrant their own chrome rather than a mini-panel. Not a v0.1 blocker.
  ...
  * [x] **`set_prob`** (per-row probability assignment): the inspector renders the stored probability for input/update gates as another row in the gate-metadata `<dl>` (formatted with the panel's "Probability decimals"). The cell is click-to-edit: click → number input → Enter fires `POST /api/set_prob` → updates in place. Esc / blur cancels. Server validates `0 ≤ p ≤ 1` and surfaces ProvSQL errors inline. Bulk set_prob via SQL (`SELECT set_prob(provenance(), col) FROM tbl`) is left to the querybox : a clicky bulk UI didn't fit cleanly given the open-ended expression form.
...
* [x] **Big-table strategy**: three caps + a server-side `LIMIT` push, all configurable from the Config panel (Display limits group) and persisted on disk:
  * **Where-mode sidebar**: per-relation `LIMIT N+1` in `list_relations` (default `--max-sidebar-rows=100`); response carries `truncated` + `estimated_rows` (from `pg_class.reltuples`); the per-relation header reads `100 of ~50000 tuples` when capped. Mapping-shaped relations excluded from the sidebar entirely (CTAS over a tracked table can leak a `provsql` column onto a `(value, provenance)` mapping ; the sidebar treats them as labelings, not source data).
  * **Result table**: server-side `cur.fetchmany(N+1)` cap on `/api/exec` (default `--max-result-rows=1000`); response carries `truncated` + `max_rows`; the meta line shows `1000 (first 1000; more available) tuples` when capped. Cleared at the start of every new query so the previous result's marker doesn't linger during loading.
  * **Where-mode wrap LIMIT push**: the `where_provenance(provenance())` projection runs once per row server-side, so without a SQL-level cap a 50 k-row SELECT in where mode took >20 s. The wrap now appends `LIMIT N+1` to the outer SELECT ; PG's pull-based executor stops the projection after N+1 rows, dropping where-mode runtime on Q1 to roughly the same range as circuit mode.
  * **Circuit mode**: default node cap dropped from 500 → 200, exposed as Config-panel knob "Nodes per fetch" (`--max-circuit-nodes`, range 10..10 000). Cap is per-fetch, not per-merged-scene : the scene grows interactively as the user expands frontiers.
  * Notice / error banners + truncation marker now reset at the start of every new query so stale state doesn't linger during loading.
...
* [ ] **Contributions mode** (third mode, alongside Where + Circuit): heat-map of per-input Shapley / Banzhaf contributions for the current result. The mode-switcher gains a third tab; the sidebar lists input gates with mapping-resolved labels and a per-input contribution bar; result-table rows get a "→ Contributions" affordance the same way they have "→ Circuit" today. Backed by `shapley_all_vars` / `banzhaf_all_vars`. The eval-strip variants (per-node `shapley` / `banzhaf` with a variable-token picker) likely fold into this mode rather than living on the circuit canvas.
* [ ] **Time-travel mode** (fourth mode): dedicated chrome for the temporal SRFs `timeslice` / `history` / `timetravel` (CS4 §3–5). Sidebar = view picker + date / window / column-filter that compose the SRF call; the result table renders the SRF output. The verbose `... AS (cols ...)` shape is the main motivation: a templated form is friendlier than asking users to write it by hand.
...
* [x] **Lazy expansion sizing**: superseded by the cap-defaults pass and the actionable 413 banner. `max_circuit_nodes` dropped 500→200 ; the banner offers "Render at depth 1, then expand interactively" when depth-1 fits, so wide circuits hit the actionable path rather than rendering a hairball. `max_circuit_depth` stays at 8.

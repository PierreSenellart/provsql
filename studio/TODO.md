...
* [ ] **Drag-to-move circuit nodes**: each `.node-group` is already a `<g transform="translate(x,y)">` so the renderer can support per-node dragging by adding a `mousedown` handler on the group, converting client deltas to SVG coords via `svg.getScreenCTM().inverse()`, updating both the node's transform and its `state.scene.nodes` entry, and re-painting the small set of edges incident to the moved node. ~80 lines in `circuit.js`. Distinguish click-to-pin from drag with a small movement threshold (~4 px). Open question: positions are ephemeral (reset on `renderCircuit` and on frontier expansion) — fine for one-off "move this Π out of the way to see the agg behind it" tweaks; if persistence matters, store a `{uuid: (dx, dy)}` offsets map per circuit root and re-apply after layout. A "Reset layout" toolbar button covers the "I made it worse" case for free.

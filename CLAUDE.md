...
1. **`sql/upgrades/provsql--<prev>--<new>.sql` MUST contain every new SQL object added since the prior release**, not only the obvious enum / type additions. The mechanical verification is **`make upgrade-parity-test`** (`test/upgrade_parity.sh`): it builds one database through `CREATE EXTENSION VERSION '1.0.0'` + `ALTER EXTENSION UPDATE` and one through a direct `CREATE EXTENSION`, and diffs their catalogs (functions with signature/body-hash/volatility, aggregates, operators with commutators, casts with their context, types, enum values, relations, and every extension member with its schema). It must report zero differences. For manual inspection:
   ```bash
   git diff v<prev>..HEAD -- sql/provsql.common.sql sql/provsql.14.sql
   ```
   Every `CREATE TYPE`, `CREATE FUNCTION`, `CREATE OPERATOR`, `CREATE CAST`, `CREATE AGGREGATE` whose name is new (or whose body / signature changed) must be replicated in the upgrade script. Use `CREATE OR REPLACE FUNCTION` freely. For DDL that does not support `OR REPLACE` (`TYPE`, `OPERATOR`, `CAST`, `AGGREGATE`), wrap each in a `DO` block that checks the appropriate `pg_catalog` table and skips on duplicate, so the script is idempotent — and when the guard is an operator-existence check, also require `oprcode <> 0`: a `COMMUTATOR`/`NEGATOR` forward reference auto-creates a SHELL operator under the same key, and treating the shell as "exists" skips the real definition for good (this shipped broken in 1.4.0→1.5.0). Upgrade scripts must start with `SET search_path TO provsql;` or their objects land in the session default schema (this shipped broken in 1.6.0→1.7.0). Signature changes need `DROP FUNCTION IF EXISTS <old sig>;` before the new `CREATE FUNCTION`. Mind dependency order: TYPE → in/out functions → CAST → constructors → arithmetic / comparison functions → OPERATOR → aggregate sfunc/ffunc → AGGREGATE → polymorphic dispatchers. **This has been forgotten in past releases**; if `provsql--<prev>--<new>.sql` is still a small stub when the rest of the SQL surface has grown, that is the bug.

...
typedef struct constants_t {
  ...
  Oid OID_FUNCTION_SUM_RV; ///< OID of provsql.sum(random_variable) aggregate (rv -> rv)
  Oid OID_FUNCTION_RV_AGGREGATE_SEMIMOD; ///< OID of rv_aggregate_semimod helper (uuid, rv -> rv) used to wrap sum(random_variable)'s per-row argument

  ...
  SemiringT S = SemiringT{})
{
  ...
  // enumerates exact worlds (the numeric comparison and array_agg): a certifying
  // semiring gets one certified world term per world under a deterministic OR;
  // otherwise each world contributes ∏present ⊗ (1 ⊖ ⊕missing), with the upset
  // and monotone MIN/MAX shortcuts dropping the absent contributors that the
  // enumeration left unconstrained.  @p op / @p agg_kind drive only those
  // shortcuts (no-ops for array_agg).
  auto combine_exhaustive_worlds =
    [&](const std::vector<mask_t> &worlds,
        const std::vector<typename SemiringT::value_type> &kvals,
        bool upset, ComparisonOperator op,
      ...
      for (const auto &mask : worlds) {
        ...
        for (size_t i = 0; i < n; ++i) {
          ...
          } else if ((op == ComparisonOperator::GE || op == ComparisonOperator::GT) &&
                     S.absorptive() && agg_kind == AggregationOperator::MAX) {
            // Monotonously increasing behavior: do not add anything to missing
          } else if ((op == ComparisonOperator::LE || op == ComparisonOperator::LT) &&
                     S.absorptive() && agg_kind == AggregationOperator::MIN) {
  ...
  auto pw_from_cmp_gate = [&](gate_t cmp_gate, typename SemiringT::value_type &pw_out) -> bool {
    ...
    auto build_from = [&](gate_t agg_side, gate_t const_side, ComparisonOperator effective_op) -> bool {
      ...
      pw_out = combine_exhaustive_worlds(worlds, kvals, upset, op, agg_kind);
      return true;
    };

...
static Const *
extract_having_literal(Node *node) { // look for constant addd check
  if (node == NULL)
    return NULL;
...
static Expr *make_provenance_expression(const constants_t *constants, Query *q,
                                        ...
                                        bool having, bool group_by_rewrite,
                                        semiring_operation op, int **columns,
                                        int nbcols, Expr *external_result_agg) {
  Expr *result;
  Expr *result_agg = NULL;
  ...
  if (op == SR_PLUS) {
    ...
    if (group_by_rewrite || aggregation) {
      ...
      result = (Expr *)plus;
      result_agg = (Expr *)plus;
    }
    // times( delta, cmp(aggregation_result,  semimod(1,having_literal)))
    if (aggregation) {
    ...
    if (q->havingQual) {
      ...
      cmpExpr->args = list_make3(external_result_agg, oid, (Expr *)semimodExpr);
      cmpExpr->location = -1;
...
// function to scan through target list of query after aggregation has been
// replaced by provenance aggregates, and returns first match to aggtokenid
static Expr *find_aggregation_expr(List *targetList, Oid aggTokenOid) {
  ListCell *lc;
  foreach (lc, targetList) {
    TargetEntry *te = (TargetEntry *)lfirst(lc);
    if (exprType((Node *)te->expr) == aggTokenOid)
      return te->expr;
  }
  return NULL;
}
...
static Query *process_query(const constants_t *constants, Query *q,
  ...
  if (supported) {
    ...
    if (q->hasAggs) {
      ...
      external_result_agg =
          find_aggregation_expr(q->targetList, constants->OID_TYPE_AGG_TOKEN);

...
# provsql.imports is regenerated by our build-pglite.sh relink (step below);
# a copy left over from a failed run would otherwise be picked up by the
# upstream base build's pglite link, which runs first -- so clear it now so
# re-runs start from the same clean state as a fresh clone.
